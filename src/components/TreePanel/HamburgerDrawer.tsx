...
const HamburgerDrawer = ({ isOpen, onClose }: HamburgerDrawerProps) => {
    ...
    return (
        <>
            ...
            <div
                ref={drawerRef}
                className={`fixed top-0 h-full w-64 bg-white dark:bg-gray-900 shadow-xl z-30 transform transition-transform duration-300 ease-in-out
    ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <nav className="p-2">
                    ...
                    ${m.active ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-medium" : "text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"}`}>
                            <span>{m.icon}</span>
                            <span>{m.label}</span>
                            {m.active && (
